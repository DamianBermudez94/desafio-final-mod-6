...
const state = {
  ...
  listenRoom() {
    const currentState = this.getState();
    const roomRef = rtdb.ref("/gamerooms/" + currentState.rtdbRoomId);
  ...
  updateDataRoom() {
    const cs = this.getState();
    return fetch("/gamerooms/" + cs.rtdbRoomId, {
      method: "PUT",
      headers: {
  ...
  accessToRoom() {
    const cs = this.getState();
    const gameroomsId = cs.gameroomsId;
    console.log(gameroomsId);
    
    return fetch("/gamerooms/" + gameroomsId + "?userId=" + cs.userId)
      .then((res) => {
        return res.json();
      })
      .then((data) => {
